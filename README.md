...
| <a name="input_replication_role_name"></a> [replication\_role\_name](#input\_replication\_role\_name) | Name of the IAM role in the hub-and-spoke account used by S3 to replicate logs to the central bucket.<br/>    The role name is defined statically in the hubandspoke/logging.tf file and must match this value. | `string` | `"hubandspoke-central-logs-replication"` | no |
