# Organizations Module

## Overview

This module establishes the foundational multi-account structure for DoD Zero Trust CSPM deployments. It manages AWS Organizations account placement, organizational unit (OU) structure, and provides critical integration points for Control Tower, SSO, and cross-account security services.

**Key Architecture Pattern**: Hybrid OU management based on `control_tower_enabled` flag - Organizations module handles standard SRA OUs while delegating Security and Sandbox OUs to Control Tower when enabled.

## Architecture Pattern

### Multi-Account Foundation
- **Account Management**: Manages existing AWS accounts (accounts must be pre-created via CLI)
- **OU Structure**: Implements AWS Security Reference Architecture (SRA) organizational units
- **Partition Support**: Automatically detects and handles both Commercial and GovCloud partitions
- **Control Tower Integration**: Conditional OU delegation based on `control_tower_enabled` setting

### OU Management Strategy
When `control_tower_enabled = true`:
- Organizations module creates: Infrastructure_Prod, Infrastructure_NonProd, Workloads_Prod, Workloads_NonProd, Policy_Staging, Suspended
- Control Tower creates: Security, Sandbox (during landing zone deployment)
- Accounts targeting Control Tower OUs are placed at Root initially, then moved by Control Tower

When `control_tower_enabled = false`:
- Organizations module creates ALL OUs defined in `organizational_units` variable
- No Control Tower constraints or account type validation requirements

### Account Type System
Implements SRA account types with Control Tower validation:
- **Required for Control Tower**: management, log_archive, audit accounts
- **Account Type Mapping**: Provides `account_role_mapping` output for SSO integration
- **Validation**: Enforces proper account placement and type consistency

## Deployment Requirements

### Prerequisites
1. **Pre-existing Accounts**: All AWS accounts must exist before deployment
   ```bash
   # GovCloud account creation example
   aws organizations create-gov-cloud-account \
     --account-name "YourOrg-Management" \
     --email "aws-mgmt@yourorg.com" \
     --profile your-management-profile
   ```

2. **Account Configuration**: Use exact names and emails from CLI creation
   ```hcl
   aws_account_parameters = {
     "ACTUAL_ACCOUNT_ID" = {
       name         = "Exact-CLI-Name"           # Must match CLI creation
       email        = "exact-email@domain.com"  # Must match CLI creation
       ou           = "Security"                 # Target OU placement
       lifecycle    = "prod"                     # prod/nonprod validation
       account_type = "management"               # Required for Control Tower integration
     }
   }
   ```

3. **Provider Setup**: Management account credentials with Organizations permissions
   ```hcl
   provider "aws" {
     profile = "your-management-profile"
     region  = "us-gov-west-1"  # GovCloud deployments
   }
   ```

### Configuration Variables
- **`control_tower_enabled`**: Boolean controlling OU management behavior and validation requirements
- **`aws_account_parameters`**: Map of account IDs to placement and type configuration
- **`organizational_units`**: OU definitions with lifecycle and tagging (uses SRA defaults)
- **`aws_organization_id`**: Optional for existing organization management

## Troubleshooting

### Common Account Placement Issues
1. **Account Not Found**: Verify account ID matches actual AWS account and account was created via CLI
2. **OU Validation Failures**: Check that target OU exists in `organizational_units` or is Control Tower-managed
3. **Control Tower Conflicts**: Ensure required account types (management, log_archive, audit) are configured when `control_tower_enabled = true`

### Partition Detection Problems
- **GovCloud Detection**: Module automatically detects partition via `data.aws_partition.current`
- **Cross-Partition Issues**: Ensure provider is configured for correct partition (Commercial vs GovCloud)

### Import Existing Resources
```hcl
import {
  to = module.organizations.aws_organizations_account.govcloud["ACCOUNT_ID"]
  id = "ACCOUNT_ID"
}
```

## Integration with Other Modules

### SSO Module Integration
- **Account Mapping**: Provides `account_id_map` output for SSO account assignments
- **Role Mapping**: Supplies `account_role_mapping` for automatic group assignments based on account types
- **Type Detection**: SSO uses account_type tags for permission set deployment

### Control Tower Module Integration
- **Account Prerequisites**: Provides required account IDs (management, log_archive, audit) via dedicated outputs
- **OU Coordination**: Manages non-Control Tower OUs while delegating Security/Sandbox to Control Tower
- **Partition Context**: Supplies partition detection for Control Tower regional requirements

### Security Services Integration
- **Foundation Accounts**: Establishes audit account as delegated administrator for security services
- **Cross-Account Structure**: Provides account inventory for organization-wide security service deployment
- **Account Organization**: Groups accounts by type via `accounts_by_type` output for service-specific configuration

## DoD-Specific Considerations

### GovCloud Deployment Patterns
- **Automatic Partition Detection**: Handles GovCloud-specific account creation and management patterns
- **Compliance Mapping**: Implements SRA account structure aligned with DoD security requirements
- **Enterprise Integration**: Supports large-scale DoD organization structures with hundreds of accounts

### Security Architecture Alignment
- **DISA SCCA Foundation**: Provides multi-account foundation required for SCCA component isolation
- **Control Tower Ready**: Establishes account structure compatible with Control Tower governance requirements
- **Audit Trail Integration**: Supports centralized logging and monitoring account patterns

### Operational Constraints
- **CLI-First Workflow**: Accounts must be created via CLI before Terraform management (DoD standard practice)
- **Management Account Strategy**: Single management account provider pattern for organizational resource management
- **Account Type Validation**: Enforces proper account categorization for compliance reporting

<!-- BEGIN_TF_DOCS -->
## Requirements

| Name | Version |
|------|---------|
| <a name="requirement_terraform"></a> [terraform](#requirement\_terraform) | >= 1.6 |
| <a name="requirement_aws"></a> [aws](#requirement\_aws) | >= 5.0.0 |

## Providers

| Name | Version |
|------|---------|
| <a name="provider_aws"></a> [aws](#provider\_aws) | 6.4.0 |

## Modules

No modules.

## Resources

| Name | Type |
|------|------|
| [aws_organizations_account.commercial](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/organizations_account) | resource |
| [aws_organizations_account.govcloud](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/organizations_account) | resource |
| [aws_organizations_organization.this](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/organizations_organization) | resource |
| [aws_organizations_organizational_unit.this](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/organizations_organizational_unit) | resource |
| [aws_organizations_organization.this](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/organizations_organization) | data source |
| [aws_partition.current](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/partition) | data source |

## Inputs

| Name | Description | Type | Default | Required |
|------|-------------|------|---------|:--------:|
| <a name="input_aws_account_parameters"></a> [aws\_account\_parameters](#input\_aws\_account\_parameters) | Map of AWS account parameters to be managed by the module.<br/><br/>PREREQUISITE: All accounts must already exist (created via AWS Organizations CLI).<br/><br/>Structure:<br/>  {<br/>    "123456789012" = {<br/>      name         = "YourCorp-Management"<br/>      email        = "aws-mgmt@yourcorp.com"<br/>      ou           = "Root"<br/>      lifecycle    = "prod"<br/>      account\_type = "management"<br/>    }<br/>  }<br/><br/>**CONTROL TOWER ACCOUNT PLACEMENT:**<br/>When control\_tower\_enabled = true:<br/>- Accounts with ou = "Security" or ou = "Sandbox" are placed at "Root" initially<br/>- Control Tower landing zone will move them to the proper Control Tower-managed OUs<br/>- Other accounts are placed in organizations-managed OUs normally<br/>- Required account types: management, log\_archive, audit (see validation below)<br/><br/>When control\_tower\_enabled = false:<br/>- All accounts are placed in their specified OUs directly<br/>- No Control Tower constraints or account type requirements<br/><br/>See config/account-schema.yaml for detailed field definitions and examples.<br/>See config/sra-account-types.yaml for valid account\_type values. | <pre>map(object({<br/>    name            = string<br/>    email           = string<br/>    ou              = string<br/>    lifecycle       = string<br/>    account_type    = optional(string, "")<br/>    create_govcloud = optional(bool, false)<br/>  }))</pre> | n/a | yes |
| <a name="input_aws_organization_id"></a> [aws\_organization\_id](#input\_aws\_organization\_id) | ID for existing AWS Govcloud Organization. If not provided, the module will create a new organization. | `string` | `null` | no |
| <a name="input_control_tower_enabled"></a> [control\_tower\_enabled](#input\_control\_tower\_enabled) | Whether Control Tower will be deployed with this organization.<br/><br/>**IMPORTANT: This significantly changes OU management behavior:**<br/><br/>When true (Control Tower mode):<br/>- Organizations module will NOT create "Security" and "Sandbox" OUs<br/>- Control Tower landing zone will create and manage these OUs instead<br/>- Accounts targeting Control Tower-managed OUs are temporarily placed at Root<br/>- Control Tower will move them to proper OUs during landing zone deployment<br/>- Enforces Control Tower account requirements (management, log\_archive, audit accounts)<br/>- Validates AccountType tags and specific OU placements<br/><br/>When false (Organizations-only mode):<br/>- Organizations module creates ALL OUs defined in organizational\_units<br/>- No Control Tower integration or constraints<br/>- AccountType tags are optional<br/>- More flexible OU assignments allowed<br/><br/>**Control Tower-managed OUs:** Security, Sandbox<br/>**Organizations-managed OUs:** Infrastructure\_Prod, Infrastructure\_NonProd, Workloads\_Prod, Workloads\_NonProd, Policy\_Staging, Suspended | `bool` | `true` | no |
| <a name="input_global_tags"></a> [global\_tags](#input\_global\_tags) | Tags applied to all resources created by this module. | `map(string)` | <pre>{<br/>  "ManagedBy": "opentofu"<br/>}</pre> | no |
| <a name="input_organizational_units"></a> [organizational\_units](#input\_organizational\_units) | Map of Organizational Unit (OU) names to their attributes.<br/><br/>Uses AWS Security Reference Architecture (SRA) standard OUs by default:<br/>- Security: For audit, log archive, and security tooling accounts<br/>- Infrastructure\_Prod/NonProd: For network and shared services accounts<br/>- Workloads\_Prod/NonProd: For application workload accounts<br/>- Sandbox: For experimentation and development<br/>- Policy\_Staging: For testing organizational policies<br/>- Suspended: For decommissioned accounts<br/><br/>**CONTROL TOWER INTEGRATION:**<br/>When control\_tower\_enabled = true:<br/>- "Security" and "Sandbox" OUs are NOT created by this module<br/>- Control Tower landing zone creates and manages these OUs<br/>- Only the remaining OUs are created by the organizations module<br/>- Accounts targeting Control Tower OUs are placed at Root initially, then moved by Control Tower<br/><br/>When control\_tower\_enabled = false:<br/>- ALL OUs defined here are created by the organizations module<br/>- Standard organizational structure with no Control Tower constraints<br/><br/>Example:<br/>  {<br/>    Security = {<br/>      lifecycle = "prod"<br/>      tags      = { Owner = "SecurityTeam" }<br/>    }<br/>  } | <pre>map(object({<br/>    lifecycle = optional(string, "prod") # Default to prod<br/>    tags      = optional(map(string), {})<br/>  }))</pre> | <pre>{<br/>  "Infrastructure_NonProd": {<br/>    "lifecycle": "nonprod"<br/>  },<br/>  "Infrastructure_Prod": {<br/>    "lifecycle": "prod"<br/>  },<br/>  "Policy_Staging": {<br/>    "lifecycle": "nonprod"<br/>  },<br/>  "Sandbox": {<br/>    "lifecycle": "nonprod"<br/>  },<br/>  "Security": {<br/>    "lifecycle": "prod"<br/>  },<br/>  "Suspended": {<br/>    "lifecycle": "nonprod"<br/>  },<br/>  "Workloads_NonProd": {<br/>    "lifecycle": "nonprod"<br/>  },<br/>  "Workloads_Prod": {<br/>    "lifecycle": "prod"<br/>  }<br/>}</pre> | no |
| <a name="input_project"></a> [project](#input\_project) | Name of the project or application. Used for resource naming and tagging. | `string` | `"CnScca"` | no |

## Outputs

| Name | Description |
|------|-------------|
| <a name="output_account_id_map"></a> [account\_id\_map](#output\_account\_id\_map) | Map of account names to account IDs for use by other modules (e.g., SSO, Control Tower). |
| <a name="output_account_organizational_units"></a> [account\_organizational\_units](#output\_account\_organizational\_units) | Map of account IDs to their OU names. |
| <a name="output_account_resources"></a> [account\_resources](#output\_account\_resources) | Map of account resources (abstracts commercial vs govcloud partition differences) |
| <a name="output_account_role_mapping"></a> [account\_role\_mapping](#output\_account\_role\_mapping) | Map of account names to their AccountType tags for use by SSO module |
| <a name="output_accounts_by_type"></a> [accounts\_by\_type](#output\_accounts\_by\_type) | Accounts organized by their SRA account type for easy module integration. |
| <a name="output_audit_account_id"></a> [audit\_account\_id](#output\_audit\_account\_id) | Account ID for the audit account (required for Control Tower) |
| <a name="output_aws_partition"></a> [aws\_partition](#output\_aws\_partition) | The AWS partition (aws or aws-us-gov) where the organization is running. |
| <a name="output_global_tags"></a> [global\_tags](#output\_global\_tags) | Global tags for use by submodules. |
| <a name="output_is_govcloud"></a> [is\_govcloud](#output\_is\_govcloud) | Boolean indicating if the organization is running in AWS GovCloud. |
| <a name="output_log_archive_account_id"></a> [log\_archive\_account\_id](#output\_log\_archive\_account\_id) | Account ID for the log archive account (required for Control Tower) |
| <a name="output_management_account_id"></a> [management\_account\_id](#output\_management\_account\_id) | Account ID for the management account (required for Control Tower) |
| <a name="output_organization_id"></a> [organization\_id](#output\_organization\_id) | The AWS Organization ID. |
| <a name="output_organizational_unit_ids"></a> [organizational\_unit\_ids](#output\_organizational\_unit\_ids) | Map of OU names to their AWS Organization Unit IDs. |
| <a name="output_project"></a> [project](#output\_project) | Project name for use by submodules. |
<!-- END_TF_DOCS -->